...
class GonGon(
    utilities.Utilities, nighsthift.NightShiftUtilities, timeloop.TimeLoop, commands.Bot
):
    def __init__(self) -> None:
        ...
        self.command(name="bombel")(self.bubble)
    ...
    def initializeState(self):
        ...
        self.nightChannels = [
            dc.utils.get(self.mainGuild.channels, id=x)
        ...
        self.nightRole = dc.utils.get(self.mainGuild.roles, id=phonebook.NIGHT_ROLE_ID)
        ...
        self.registerOnTimeEvent(
            self.nightTimeStartHour,
            self.nightTimeStartMinute,
            self.announceNightTimeBegin,
        )
        self.registerOnTimeEvent(2, 00, self.announceNightTimeMiddle)
        self.registerOnTimeEvent(5, 50, self.announceNightTimeEnd)
    ...
    async def on_ready(self):
        ...
        await self.messageCreator(
            self.getHello()
            + " Wstałem."
            + (" ale debugowo" if DEV else " ale produkcyjnie")
        )
    ...
    async def on_message(self, message):
        ...
        if (
            message.guild == self.mainGuild
            and message.author.id == phonebook.GONCIARZ_ID
            and time() - self.lastFunMessage > 60 * 60 * 24
        ):
